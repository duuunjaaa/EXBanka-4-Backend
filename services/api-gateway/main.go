...
func main() {
	...
	rlSecurities := middleware.RateLimit(redisClient, 60, time.Minute)
	rlExchange := middleware.RateLimit(redisClient, 30, time.Minute)
	rlOrders := middleware.RateLimit(redisClient, 20, time.Minute)
	rlAuth := middleware.RateLimit(redisClient, 120, time.Minute)
	...
	r.POST("/login", rlAuth, handlers.Login(authClient))
	r.POST("/refresh", rlAuth, handlers.Refresh(authClient))
	r.POST("/client/login", rlAuth, handlers.ClientLogin(authClient))
	r.POST("/client/refresh", rlAuth, handlers.ClientRefresh(authClient))
	r.GET("/client/me", handlers.GetMe(clientClient))
	r.POST("/auth/activate", rlAuth, handlers.Activate(authClient))
	r.POST("/auth/forgot-password", rlAuth, handlers.ForgotPassword(authClient, emailClient))
	r.POST("/auth/reset-password", rlAuth, handlers.ResetPassword(authClient))
	r.POST("/auth/logout", rlAuth, handlers.Logout(authClient))
	...
	r.GET("/exchange/rates", rlExchange, handlers.GetExchangeRates(exchangeClient))
	r.GET("/exchange/rate", rlExchange, handlers.GetExchangeRate(exchangeClient))
	r.POST("/exchange/convert", rlExchange, handlers.ConvertAmount(exchangeClient))
	r.GET("/exchange/history", rlExchange, handlers.GetExchangeHistory(exchangeClient))
	r.POST("/exchange/preview", rlExchange, handlers.PreviewConversion(exchangeClient))
	...
	r.GET("/securities", rlSecurities, handlers.GetSecurities(securitiesClient))
	r.GET("/securities/:id", rlSecurities, handlers.GetSecurityById(securitiesClient))
	r.GET("/securities/:id/history", rlSecurities, handlers.GetSecurityHistory(securitiesClient))
	...
	r.POST("/orders", rlOrders, middleware.RequireRole("AGENT", "SUPERVISOR"), handlers.CreateOrder(orderClient, fundClient))
	r.POST("/client/orders", rlOrders, handlers.CreateOrder(orderClient, fundClient))
	r.GET("/client/orders/:id", rlOrders, handlers.GetOrderById(orderClient))
	r.DELETE("/client/orders/:id", rlOrders, handlers.CancelOrder(orderClient))
	r.DELETE("/client/orders/:id/portions", rlOrders, handlers.CancelOrderPortions(orderClient))
	r.GET("/orders", rlOrders, middleware.RequireRole("SUPERVISOR"), handlers.ListOrders(orderClient, employeeClient, securitiesClient))
	r.GET("/orders/:id", rlOrders, middleware.RequireRole("AGENT", "SUPERVISOR"), handlers.GetOrderById(orderClient))
	r.PUT("/orders/:id/approve", rlOrders, middleware.RequireRole("SUPERVISOR"), handlers.ApproveOrder(orderClient))
	r.PUT("/orders/:id/decline", rlOrders, middleware.RequireRole("SUPERVISOR"), handlers.DeclineOrder(orderClient))
	r.DELETE("/orders/:id/portions", rlOrders, middleware.RequireRole("AGENT", "SUPERVISOR"), handlers.CancelOrderPortions(orderClient))
	r.DELETE("/orders/:id", rlOrders, middleware.RequireRole("AGENT", "SUPERVISOR"), handlers.CancelOrder(orderClient))
